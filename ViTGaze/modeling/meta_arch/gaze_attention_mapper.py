...
class GazeAttentionMapper(nn.Module):
    ...
    def forward(self, x):
        ...
        if feats is not None:
            if embedded_heads.shape[-2:] != feats.shape[-2:]:
                embedded_heads = F.interpolate(
                    embedded_heads,
                    size=feats.shape[-2:],
                    mode="nearest",
                )
        ...
        if embedded_heads.shape[-2:] != attn_maps.shape[-2:]:
            embedded_heads = F.interpolate(
                embedded_heads,
                size=attn_maps.shape[-2:],
                mode="nearest",
            )
        x["attention_maps"] = (
    ...
    def preprocess_inputs(self, batched_inputs: Dict[str, torch.Tensor]):
        return (
            ...
            batched_inputs["object_target_masks"].to(self.device)
            if "object_target_masks" in batched_inputs.keys()
            else None,
            batched_inputs["object_has_targets"].to(self.device)
            if "object_has_targets" in batched_inputs.keys()
            else None,
        )
