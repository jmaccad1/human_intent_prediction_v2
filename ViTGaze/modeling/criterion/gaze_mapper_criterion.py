...
class GazeMapperCriterion(nn.Module):
    ...
    def forward(
        ...
        gt_head_masks=None,
        gt_object_masks=None,
        gt_object_has_targets=None,
    ):
        loss_dict = {}
        ...
        if (
            gt_object_masks is not None
            and gt_object_has_targets is not None
            and self.object_weight > 0
        ):
            object_loss = self.object_heatmap_loss(
                pred_heatmap,
                gt_object_masks,
                gt_object_has_targets,
            )
            loss_dict["object target loss"] = object_loss * self.object_weight

        return loss_dict

    @staticmethod
    def object_heatmap_loss(pred_heatmap, gt_object_masks, gt_object_has_targets):
        if pred_heatmap.shape[-2:] != gt_object_masks.shape[-2:]:
            pred_heatmap = F.interpolate(
                pred_heatmap,
                size=tuple(gt_object_masks.shape[-2:]),
                mode="bilinear",
                align_corners=True,
            )

        target_flags = gt_object_has_targets.reshape(-1).float()
        if torch.sum(target_flags) <= 0:
            return pred_heatmap.sum() * 0

        pred_probs = F.softmax(pred_heatmap.squeeze(1).flatten(1), dim=1).reshape_as(
            gt_object_masks
        )
        target_ratio = (pred_probs * gt_object_masks).sum(dim=(-2, -1)).clamp_min(1e-6)
        loss = -torch.log(target_ratio) * target_flags
        return loss.sum() / target_flags.sum()
