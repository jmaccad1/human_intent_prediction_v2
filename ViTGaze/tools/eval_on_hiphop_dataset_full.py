...
def _force_hiphop_full_test_paths(cfg, dataset_root):
...
def do_test(cfg, model, use_dark_inference=False):
    ...
    if not auc_values:
        raise RuntimeError(
            "No in-frame gaze samples were found in hiphop_dataset_full test."
        )
...
def main(args):
    ...
    _force_hiphop_full_test_paths(cfg, dataset_root)
    cfg.dataloader.val.skip_frame = args.skip_frame
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a HIPHOP-full-finetuned ViTGaze model on the "
            "hiphop_dataset_full test split."
        )
    )
    parser.add_argument(
        "--config_file",
        "--config-file",
        default=_repo_path("configs", "hiphop_dataset_full_6gb.py"),
        help="HIPHOP full config file.",
    )
    parser.add_argument(
        "--model_weights",
        "--model-weights",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Evaluate every Nth frame from each HIPHOP annotation sequence.",
    )
