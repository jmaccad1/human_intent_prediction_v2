...
def smooth_by_conv(window_size, df, col):
    ...
    padded_track = pd.concat(
        [
            pd.Series([df.iloc[0][col]] * half_window),
            df[col].reset_index(drop=True),
            pd.Series([df.iloc[-1][col]] * half_window),
        ],
        ignore_index=True,
    ).to_numpy()

    #added kern + changed mode from 'valid' to 'same' --- IGNORE ---
    kern = np.ones(window_size) / window_size

    smoothed_signals = np.convolve(padded_track, kern, mode="valid")
    #print(smoothed_signals.shape) 
