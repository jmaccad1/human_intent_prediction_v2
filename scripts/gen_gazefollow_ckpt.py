...
if __name__ == "__main__":
    ...
    #Start Point for Multi Session Head BBox Predicting for Low Level GPUs
    ctr = 36
    current = ctr
    ...
    for path in tqdm.tqdm(paths, desc="Predicting head bboxes: "):
        ...
        if current != int(path[7:14]):
            ...
            df.to_csv(os.path.join(args.dataset_dir, f"{args.subset}_head1.csv"), index=False, header=False, mode='a')
            csv = []
    ...
    df.to_csv(os.path.join(args.dataset_dir, f"{args.subset}_head1.csv"), index=False, header=False)
    '''
